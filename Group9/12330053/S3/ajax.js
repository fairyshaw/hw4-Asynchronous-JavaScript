...
function loaded() {
    ...
    apb[0].setAttribute("onclick", "robot3()");
}
...
function robot3(){
    ...
    var len = children.length;
    for (var c = 0;c<len;c++){
        var cur = children[c].getElementsByClassName("unread");
        if (cur.length == 0){//如果这个按钮还没有被按下
            robot3Ajax(children[c]);
        }
    }
    //calculate();
}
...
function robot3Ajax(target){
    ...
    disableOtherBtn(target);
    robot3CallRandom(target);
}

function robot3CallRandom(target){
    var XHR;
    if (window.XMLHttpRequest) {// code for IE7+, Firefox, Chrome, Opera, Safari
        XHR = new XMLHttpRequest();
    }
